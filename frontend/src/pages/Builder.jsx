...
// Custom hook for the build countdown
const useBuildTimer = (isActive, platform) => {
  const initialTime = platform === 'web' ? 3 : platform === 'android' ? 45 : 52;
  ...
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      setTimeLeft(initialTime); // Reset when not active
    }
            ...
            <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16, marginBottom: 24 }}>
                {(activeJob.platforms || []).map(pl => (
                  <PlatformCard key={pl} pl={pl} activeJob={activeJob} />
                ))}
              </div>
